...
2종에서 가져오고, 수업 자료는 편집 가능한 **한글(HWPX) 문서**로 나옵니다 (브라우저에서
바로 보는 HTML 미리보기 포함).
...
산출은 **한글(HWPX) 문서**이고, 브라우저에서 바로 보는 HTML 미리보기가 함께 나옵니다
(표준 라이브러리만으로 OWPML을 직접 생성 — 추가 의존성 없음).
...
MCP), 과목 pedagogy(NGSS 3차원 → 2022 개정 내용 체계 3범주), 교사 대면 언어, 산출
형식(docx → 한글 HWPX + HTML). 산출 스키마와 HTML 렌더러는 원본 그대로라 업스트림을 계속
추적할 수 있습니다.
